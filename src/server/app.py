...
if 'server' not in map(lambda i: i.lower(), core.SERVER_CONFIG['headers'].keys()):
    core.SERVER_CONFIG['headers']['Server'] = f'MocaVirtualDM({core.VERSION})'
...
async def before_server_start(app_: Sanic, loop):
    ...
    app_.app_client_config: mzk.MocaSynchronizedJSONDictFile = mzk.MocaSynchronizedJSONDictFile(
        core.APP_CLIENT_CONFIG_FILE, manual_reload=True
    )
    ...
    try:
        ...
        mzk.sys_exit(1)
    try:
        app_.mail = mzk.MocaMail(
            core.MAIL_CONFIG['smtp_server']['user'],
            core.MAIL_CONFIG['smtp_server']['password'],
            core.MAIL_CONFIG['smtp_server']['from_address'],
            core.MAIL_CONFIG['smtp_server']['ssl'],
            core.MAIL_CONFIG['smtp_server']['host'],
            int(core.MAIL_CONFIG['smtp_server']['port'])
        )
    except KeyError as e:
        mzk.print_error(f'Mail configuration error. missing key: {e}')
        mzk.sys_exit(1)
...
app.static('/moca-virtual-dm/static', str(core.STATIC_DIR), stream_large_files=core.SERVER_CONFIG['stream_large_files'])
...
# Blueprint
app.blueprint(Blueprint.group(*blueprints, url_prefix='/moca-virtual-dm'))
